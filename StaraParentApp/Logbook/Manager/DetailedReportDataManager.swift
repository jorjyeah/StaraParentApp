//
//  DetailedReportDataManager.swift
//  StaraParentApp
//
//  Created by George Joseph Kristian on 28/12/19.
...
//                        print(baseProgramsRecordID)
        ...
        CKContainer(identifier: "iCloud.com.jorjyeah.FinalChallengeAppNew").publicCloudDatabase.add(operation)
    }
    
    class func getDetailedActivity(activityRecordID : [CKRecord.ID], onComplete: @escaping ([DetailedReportModel]) -> Void){
        // second
        var activityReportModel = [DetailedReportModel]()
        ...
        database.perform(query, inZoneWith: nil) { (records, error) in
            if let error = error {
                ...
                records?.forEach({ (record) in
                    let model = DetailedReportModel(record: record)
